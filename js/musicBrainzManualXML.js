...
function searchMusicManual() {
  // Clear out previous results and hide table
  ...
  .then(stuff => {
    ...
    const xml = parser.parseFromString(stuff, "application/xml");
    console.log(xml.documentElement);
    searchResults(xml.documentElement.firstChild.childNodes)
  })
  .catch(console.error);
};
...
function searchResults(artists) {
  ...
  console.log(artists)
  for (i in artists) {
    console.log(i + ' ' + artists[i].id);
    console.log(i + ' ' + artists[i].childNodes[0].textContent);

    var newAnchor = document.createElement("a");
    // Cheat by invoking JS via anchor, pass artist ID and Name as parameters.
    newAnchor.href = `javascript:displayAlbums('${artists[i].id}', '${artists[i].childNodes[0].textContent}')`;
    // Simple block display of links (as opposed to list)
    newAnchor.className = 'd-block'; //setAttribute('class', 'd-block');
    newAnchor.innerText = artists[i].childNodes[0].textContent;
...
function displayAlbums(id, artistName) {
  ...
  var albumsURL = `https://musicbrainz.org/ws/2/release-group?artist=${id}&limit=50&fmt=json;`;
  ...
  function getAlbums(data) {
    ...
    for (i in releases) {
      if (releases[i]["secondary-types"] != "Live") { // omit live albums

        // Attempt to sort
        /* var newItem = { releaseDate: releases[i]["first-release-date"], albumTitle: releases[i]["title"]}
        dataMap.push(newItem);
        dataMap.sort((firstItem, secondItem) => firstItem.albumTitle - secondItem.albumTitle); */
